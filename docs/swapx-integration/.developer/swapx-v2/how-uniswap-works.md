...
Uniswap is an Automated Liquidity Protocol driven by the Constant Product Formula ($$1`$) and implemented through a system of non-upgradeable smart contracts on the Ethereum blockchain. It champions decentralization, censorship resistance, and security by removing the need for trusted intermediaries.
...
* The token pair acts as an AMM, ready to swap one token for another as long as the Constant Product Formula is maintained: $`$1$.
* $$1$ and $$1$ are the reserve balances, and $$1$ (the invariant) is their product, which ideally remains constant from a trade's perspective.
...
* Uniswap charges a 0.30% fee on trades, which is added to the reserves. This effectively increases $$1$ with every transaction.
