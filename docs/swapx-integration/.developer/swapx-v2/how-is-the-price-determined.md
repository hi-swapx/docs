...
The primary rule is the constant product formula ($$1`$).
...
Instead, the V2 pair contract simply checks _after_ each trade that the invariant ($`$1`$) is satisfied (accounting for fees).
...
* It then executes the swap using the router, which guarantees the swap will execute at a rate no worse than $`$1`$ difference from the observed in-block rate.
* $`$1$ is the user-specified slippage tolerance (which defaults to 0.5%).
