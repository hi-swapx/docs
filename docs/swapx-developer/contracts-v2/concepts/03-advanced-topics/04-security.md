...
Between January 8 and April 30, a team of six engineers reviewed and formally verified crucial components of the smart contracts for SwapX V2.
...
The report also has a "Design Comments" section that provided a deep technical understanding of some of the choices made in SwapX V2.
...
## Considerations when building on SwapX

When integrating SwapX V2 into another on-chain system, particular care must be taken to avoid security vulnerabilities, avenues for manipulations, and the potential loss of funds.
...
There are two primary categories of risk associated with SwapX V2. The first involves so-called "static" errors. These can include sending too many tokens to a pair during a swap (or requesting too few tokens back) or allowing transactions to linger in the mempool long enough for the sender's expectations about prices to no longer be accurate.
