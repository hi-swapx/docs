...
Instead, SwapX V2 adds this end-of-block price to a single cumulative-price variable in the core contract weighted by the amount of time this price existed. **This variable represents a sum of the SwapX price for every second in the entire history of the contract.**
...
- For a simple TWAP, the cost of manipulation increases (approx. linear) with liquidity on SwapX, as well as (approx. linear) with the length of time over which you average.
