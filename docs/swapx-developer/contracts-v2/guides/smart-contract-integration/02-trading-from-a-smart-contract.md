...
```solidity
require(DAI.approve(address(SwapXV2Router02), amountIn), 'approve failed.');
```
...
path[0] = address(DAI);
path[1] = SwapXV2Router02.WETH();
SwapXV2Router02.swapExactTokensForETH(amountIn, amountOutMin, path, msg.sender, block.timestamp);
```
...
However, when an off-chain price can't be used, an on-chain oracle should be used instead. Determining the best oracle for a given situation is not a part of this guide, but for more details on the SwapX V2 approach to oracles, see [Oracles](././concepts/core-concepts/oracles).
