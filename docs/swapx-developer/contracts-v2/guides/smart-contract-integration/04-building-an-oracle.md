To build a price oracle on SwapX V2, you must first understand the
...
that you can compute using the SwapX cumulative price variable.
...
`SwapXV2OracleLibrary#currentCumulativePrices`
...
The `SwapXV2Pair` cumulative price variables are designed to eventually overflow,
...
this repository by Keydonix.

Keydonix has developed a general purpose price feed oracle built on SwapX v2 that supports arbitrary time windows (up to 256 blocks) and doesn't require any active maintenance.
