...
interacts with the SwapX Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the SwapX V2 contracts, you should use the npm artifacts we deploy containing the core and
...
[`@swapx-lib/v2-core`](https://www.npmjs.com/package/@swapx-lib/v2-core) and
[`@swapx-lib/v2-periphery`](https://www.npmjs.com/package/@swapx-lib/v2-periphery) packages.

```shell script
npm i --save @swapx-lib/v2-core
npm i --save @swapx-lib/v2-periphery
```

If you check the `node_modules/@swapx` directory, you can now find the SwapX V2 contracts.

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@swapx-lib/v2-core/contracts
SwapXV2ERC20.sol    SwapXV2Pair.sol     libraries/
SwapXV2Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@swapx-lib/v2-periphery/contracts/
SwapXV2Migrator.sol  examples/              test/
SwapXV2Router01.sol  interfaces/
SwapXV2Router02.sol  libraries/
```
...
Now let's start with the constructor. You need to know where the `SwapXV2Factory` is deployed in order to compute the
...
The [`SwapXV2Library`](././reference/smart-contracts/library) has some helpful methods for this.

```solidity
pragma solidity ^0.6.6;

import './interfaces/ILiquidityValueCalculator.sol';
import '@swapx-lib/v2-periphery/contracts/libraries/SwapXV2Library.sol';
import '@swapx-lib/v2-core/contracts/interfaces/ISwapXV2Pair.sol';

contract LiquidityValueCalculator is ILiquidityValueCalculator {
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        ISwapXV2Pair pair = ISwapXV2Pair(SwapXV2Library.pairFor(factory, tokenA, tokenB));
...
Note that you should only deploy the precompiled SwapX contracts in the `build` directories for unit tests.
...
This is problematic because in SwapX V2 we use the hash of the bytecode in the v2-periphery
`SwapXV2Library`,
to compute the pair address.

To get the bytecode for deploying SwapXV2Factory, you can import the file via:

```javascript
const SwapXV2FactoryBytecode = require('@swapx-lib/v2-core/build/SwapXV2Factory.json').bytecode
```
