...
This guide will build off our quoting guide and show how to use a quote to construct and execute a trade on the SwapX V3 protocol. It is based on the Trading code example, found in the SwapX code examples repository. To run this example, check out the guide's README and follow the setup instructions.
...
Like in the previous guide, our example uses a config file  to configurate the inputs used.
...
```typescript
import { Route } from '@swapx-lib/v3-sdk'
...
```typescript
import { SwapQuoter } from '@swapx-lib/v3-sdk'
import { CurrencyAmount, TradeType } from '@swapx-lib/sdk-core'
...
You can find the approval function the repository. 
...
```typescript
import { SwapOptions } from '@swapx-lib/v3-sdk'
import { Percent } from '@swapx-lib/sdk-core'
...
```typescript
import { SwapRouter } from '@swapx-lib/v3-sdk'
...
The resulting example allows for trading between any two ERC20 tokens, but this can be suboptimal for the best pricing and fees. To achieve the best possible price, we use the SwapX auto router to route through pools to get an optimal cost. Our routing guide will show you how to use this router and execute optimal swaps.
