...
This guide will cover how to get the current quotes for any token pair on the SwapX protocol. It is based on the Quoting code example, found in the SwapX code examples repository. To run this example, check out the examples's README and follow the setup instructions.
...
- [`@swapx-lib/v3-sdk`](https://www.npmjs.com/package/@swapx-lib/v3-sdk)
- [`@swapx-lib/sdk-core`](https://www.npmjs.com/package/@swapx-lib/sdk-core)
...
The default config of the example uses a local fork of mainnet. If you haven't already, check out our local development guide.
To change the rpc endpoint or the Pool used, edit the `Currentconfig`.
...
```typescript
import { computePoolAddress } from '@swapx-lib/v3-sdk' 
...
These parameters have already been defined in our constants.ts file:
...
We get access to the contract's ABI through the [@swapx-lib/v3-core](https://www.npmjs.com/package/@swapx-lib/v3-core) package, which holds the core smart contracts of the SwapX V3 protocol:

```typescript
import ISwapXV3PoolABI from '@swapx-lib/v3-core/artifacts/contracts/interfaces/ISwapXV3Pool.sol/ISwapXV3Pool.json'
```
...
For the full code, check out `getPoolConstants()` in `quote.ts`.
...
Check out the full code for the following snippets in quote.ts
...
We get access to the contract's ABI through the [@swapx-lib/v3-periphery](https://www.npmjs.com/package/@swapx-lib/v3-periphery) package, which holds the periphery smart contracts of the SwapX V3 protocol:

```typescript
import Quoter from '@swapx-lib/v3-periphery/artifacts/contracts/lens/Quoter.sol/Quoter.json'
```
...
We get the QUOTE_V2_CONTRACT_ADDRESS for our chain from V3 reference deployments.

We get access to the contract's ABI through the [@swapx-lib/v3-periphery](https://www.npmjs.com/package/@swapx-lib/v3-periphery) package, which holds the periphery smart contracts of the SwapX V3 protocol:

```typescript
import Quoter as QuoterV2 from '@swapx-lib/v3-periphery/artifacts/contracts/lens/QuoterV2.sol/QuoterV2.json'
```
...
1. **AmountOut**: The tokens or cryptocurrency received in a SwapX swap transaction.

2. **Gas Estimate**: Estimated amount of gas required for executing a SwapX transaction on the Ethereum network.

3. **InitializedTicksCrossed**: In SwapX V3, indicates whether liquidity has crossed certain price thresholds within a liquidity range.

4. **SqrtPriceX96After**: Square root of the price after a SwapX V3 transaction, essential for determining pricing accuracy.
