...
This guide will cover how to modify a liquidity position by adding or removing liquidity on the SwapX V3 protocol. It is based on the modifying a position code example, found in the SwapX code examples repository. To run this example, check out the examples's README and follow the setup instructions.
...
The example can be configured in the `config.ts` file.
...
```typescript
import { AddLiquidityOptions } from '@swapx-lib/v3-sdk'
...
```typescript
import { NonfungiblePositionManager } from '@swapx-lib/v3-sdk'
...
```typescript
import { RemoveLiquidityOptions } from '@swapx-lib/v3-sdk'
...
```typescript
import { CurrencyAmount } from '@swapx-lib/sdk-core'
import { CollectOptions } from '@swapx-lib/v3-sdk'
