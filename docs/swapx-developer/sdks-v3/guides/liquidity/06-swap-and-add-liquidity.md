...
Then we can setup our router, the `AlphaRouter`, which is part of the [smart-order-router package](https://www.npmjs.com/package/@swapx/smart-order-router). The router requires a `chainId` and a `provider` to be initialized. Note that routing is not supported for local forks, so we will use a mainnet provider even when swapping on a local fork:
...
```typescript
import { Pool, Position, nearestUsableTick } from '@swapx-lib/v3-sdk'
...
```typescript
import { Fraction } from '@swapx-lib/sdk-core'
