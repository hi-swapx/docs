...
Like the Liquidity Position guide it doesn't have an accompanying example, nevertheless the concepts and functions used here can be found among the various examples that interact with liquidity positions.
...
For this guide, we do not need to use the SwapX SDKs, we will only import the contract ABI for the NonfungiblePositionManager Contract from [`@swapx-lib/v3-periphery`](https://www.npmjs.com/package/@swapx-lib/v3-periphery).
...
import INONFUNGIBLE_POSITION_MANAGER from '@swapx-lib/v3-periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json'
