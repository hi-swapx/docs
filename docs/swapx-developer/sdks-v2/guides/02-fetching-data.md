...
```typescript
import { ChainId, Token } from '@swapx-lib/sdk-core'
...
Finally, we can talk about **symbol** and **name**. Because these fields aren't used anywhere in the SDK itself, they're optional, and can be provided if you want to use them in your application. However, the SDK will not fetch them for you
...
```typescript
import { ChainId, Token, WETH9, CurrencyAmount } from '@swapx-lib/sdk-core'
...
async function createPair(): Promise<Pair> {
  ...
  // Setup provider, import necessary ABI..
  const pairContract = new ethers.Contract(pairAddress, swapxV2poolABI, provider)
