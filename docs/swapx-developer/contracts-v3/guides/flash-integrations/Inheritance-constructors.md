...
```solidity
ISwapXV3FlashCallback(msg.sender).swapxV3FlashCallback(fee0, fee1, data);
```
...
In V3 there are three separate callback functions, `swapxV3SwapCallback`, `swapxV3MintCallback`, and `swapxV3FlashCallback`, each available to be overridden with custom logic. To write our arbitrage contract, we'll be calling `flash` and overriding the `swapxV3FlashCallback` with the steps needed to finish executing our transaction.

## Inheriting The V3 Contracts

Inherit `ISwapXV3FlashCallback` and `PeripheryPayments`, as we will use each in our program. Note these two inherited contracts already extend many other contracts that we will be using, such as LowGasSafeMath which we [attach](https://docs.soliditylang.org/en/v0.7.6/contracts.html?highlight=using#using-for), to types `uint256` and `int256`.

```solidity
contract PairFlash is ISwapXV3FlashCallback, PeripheryPayments {
...
import '@swapx-lib/v3-core/contracts/interfaces/callback/ISwapXV3FlashCallback.sol';
import '@swapx-lib/v3-core/contracts/libraries/LowGasSafeMath.sol';

import '@swapx-lib/v3-periphery/contracts/base/PeripheryPayments.sol';
import '@swapx-lib/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@swapx-lib/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@swapx-lib/v3-periphery/contracts/libraries/CallbackValidation.sol';
import '@swapx-lib/v3-periphery/contracts/libraries/TransferHelper.sol';
import '@swapx-lib/v3-periphery/contracts/interfaces/ISwapRouter.sol';
